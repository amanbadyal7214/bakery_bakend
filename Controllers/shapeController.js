...
exports.createShape = async (req, res, next) => {
  try {
    const { name, shapeType, description, categories } = req.body;
    const shape = new Shape({ name, shapeType, description, categories: categories || [] });
    await shape.save();
...
exports.getShapes = async (req, res, next) => {
  try {
    const shapes = await Shape.find().populate('categories').sort({ createdAt: -1 });
    res.json(shapes);
  } catch (err) {
    next(err);
  }
...
exports.getShape = async (req, res, next) => {
  try {
    const shape = await Shape.findById(req.params.id).populate('categories');
    if (!shape) {
...
exports.updateShape = async (req, res, next) => {
  try {
    ...
    const shape = await Shape.findByIdAndUpdate(
      req.params.id,
      { name, shapeType, description, categories: categories || [] },
